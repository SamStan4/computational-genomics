...
void suffix_tree::constructor_helper() {
    // Hot pointer is the most recently inserted leaf node
    ...
    for (i32 i = 0; i <= (i32)this->m_string.size(); ++i) {
        ...
        // } else {
        //     // suffix_link = this->node_hops(hot_ptr->m_parent_ptr->m_parent_ptr);
        //     // hot_ptr->m_parent_ptr->m_suffix_ptr = suffix_link;
        //     // this->find_path_and_insert(suffix_link, i + suffix_link->m_depth, suffix_link->m_depth, hot_ptr);
        //     this->find_path_and_insert(this->m_root_ptr, i, 0, hot_ptr);
        // }
    }
...
suffix_tree_node* suffix_tree::node_hops(suffix_tree_node* start_ptr) {
    i32 i = start_ptr->m_start;
    i32 j = start_ptr->m_end;
    suffix_tree_node* cur_ptr = start_ptr->m_suffix_ptr;
    i += cur_ptr->m_end - cur_ptr->m_start;
    while (i < j) {
        cur_ptr = cur_ptr->get_pointer(this->m_string, i);
        i += cur_ptr->m_end - cur_ptr->m_start;
    }
    return cur_ptr;
}
...
void suffix_tree::get_number_internals_helper(suffix_tree_node* cur_ptr, i32& num_internals) {
    if (!cur_ptr) {
        return;
    } else if (!cur_ptr->is_leaf()) {
        ++num_internals;
    ...
    this->get_number_internals_helper(cur_ptr->m_sibling_ptr, num_internals);
    this->get_number_internals_helper(cur_ptr->m_child_ptr, num_internals);
}

i32 suffix_tree::get_number_internas() {
    i32 res = 0;
    this->get_number_internals_helper(this->m_root_ptr, res);
    return res;
}